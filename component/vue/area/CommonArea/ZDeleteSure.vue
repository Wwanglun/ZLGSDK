...
<script>
import VDialogButton from "./ZDialogButton.vue"
import ZLGUser from '../../../../sdk/js/api/http/httpModules/ZLGUser.js'
// import { surePassword } from '@/assets/js/data/history/HistoryData.js'
export default {
  components: {
    VDialogButton
  },
  ...
  data() {
    ...
    return {
      ...
      passwordForm: {
        username: !!userInfo
          ? userInfo.role === 6
            ? userInfo.username + "@" + userInfo.owner
            : userInfo.username
          : '',
        password: ""
      },
  ...
  methods: {
    submitForm() {
      ...
      this.$refs.passwordForm.validate(valid => {
        if (valid) {
          ZLGUser.login(this.passwordForm).then(res => {
            this.loading = false
            this.$emit("sureDel", this.isClear)
            this.cancel()
          }).catch(err => {
            this.loading = false
            this.$message({message: "密码错误", customClass: "login-tip", type: "error", duration: 3200})
          })
        } else {
          this.loading = false
          return false
