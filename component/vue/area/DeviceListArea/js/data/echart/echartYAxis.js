import {timestampToTimeForService} from '../util/utils'
import { timeTodays, timesToMonths, timesToYears } from '../echart/echartTime'
  ...
  if (params instanceof Array) {
    params.forEach((v, i) => {
      if (dataPoint.length > 0 && xAxisData.length === 0) {
        ...
        if (timeShowFormat === 0) str = `<div class="myChartTip echart-toolDiv">时间: <span class="time">${timestampToTimeForService(parseInt(params[0].data[0] / 1000))}</span></div>`
        ...
        params.forEach((v, i) => {
          str += `<div class="myChartTip echart-toolDiv">${v.seriesName}: <span class="title">${dataEmpty ? '暂无数据' : v.data[1] === '0.000' ? '暂无数据' : v.data[1]}</span> ${dataEmpty || !onlyOne ? '' : unit[i] === '' ? '' : '(' + unit[i] + ')'}</div>`
        })
        ...
        if (existDevEventType === '消息' && xAxisData.length > 0) {
          ...
          if (timeShowFormat === 0) str = `<div class="myChartTip">时间: <span class="time">${timestampToTimeForService(parseInt(params[0].data[0] / 1000))}</span></div>`
