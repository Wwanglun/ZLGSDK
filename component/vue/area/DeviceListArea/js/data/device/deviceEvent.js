...
import { timestampToTimeForService, transferenceStr } from '../util/utils.js'
import {getProjectDeviceList} from '../project/project.js'
export class EventList {
  ...
  getEventsData (getters, size, currentPage, allDevice) {
    let eventsData = []
    return ZlgCloudHelper.queryDeviceEvents({ devid: JSON.parse(getters).devid, devtype: JSON.parse(getters).devtype, filter: getters, descend: true, limit: size, skip: (currentPage - 1) * size }).then(res => {
      res && res.data.forEach(value => {
        let single = JSON.parse(JSON.stringify(value))
        single.time = timestampToTimeForService((value.time + '').length === 10 ? value.time : parseInt(value.time / 1000))
        let names = getProjectDeviceList([`${value.devid},${value.devtype}`], allDevice)
        if (names.length > 0) {
  ...
  getEventsCount (getters) {
    return ZlgCloudHelper.queryDeviceEvents({ devtype: JSON.parse(getters).devtype, filter: getters, aggregation: 'count', devid: JSON.parse(getters).devid }).then(res => {
      return res.count
    })
  }
  /**
